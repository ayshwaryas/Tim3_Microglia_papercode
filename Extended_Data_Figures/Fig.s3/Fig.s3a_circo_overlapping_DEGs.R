...
gene_list_phago <- c("Havcr2", "Cd9", "Cst7", "Cstb", "Cxcl16", "Ly9", "Lyz2", 
                     unique(subset(Figs3a_DEG_dir, str_detect(gene_symbol, "H2\\-"))$gene_symbol))
...
# save(n_overlap_perm, p_overlap_perm, file = "results/Fig.s3a_perm_test_results.05.RData")

# Circos plot ------------------------------------------------------------------

Figs3a_circos <- function(
...
Figs3a_circos(DEG_overlaps = Figs3a_DEG_dir,  p_overlap_perm = p_overlap_perm,
              highlight_genes = c(gene_list_phago, gene_list_Tgfbr2, "Apoe", "Sall1", "Cd33"),
              dir_cex = 0.77, y_dir = -0.3, 
              p.perm_thres = 1, fig_num = "s3a")