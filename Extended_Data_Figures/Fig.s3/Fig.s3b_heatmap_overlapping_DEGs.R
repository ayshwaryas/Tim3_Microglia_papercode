...
Figs3b_DEG_dir_wide <- list(`Tim3KO` = tim3_DEG, `phago+` = phago_DEG,
...
Tim3KO_TPM <- read.csv("data/expr_mat/Danyang_TPM_matrix.csv", check.names = FALSE) %>%
  ...
  inner_join(Figs3b_DEG_dir_wide %>% select(gene_symbol, direction = Tim3KO), 
             by = "gene_symbol") %>%
...
phago_TPM <- read.csv("data/expr_mat/tim3_Kimi_tpm_with_genesymbols_KK.csv") %>%
  ...
  inner_join(Figs3b_DEG_dir_wide %>% select(gene_symbol, direction = `phago+`), 
             by = "gene_symbol") %>%
...
TGFBRII_FPKM <- read.csv("results/bulkRNAseq_results_TGFBRII_Lund_2018.csv") %>%
  ...
  inner_join(Figs3b_DEG_dir_wide %>% select(gene_symbol, direction = Tgfbr2KO), 
             by = "gene_symbol") %>%
...
Clec7a_FPKM <- read.csv("data/bulkRNAseq_results_Clec7a_Krasemann_2017.csv") %>%
  ...
  select(-direction) %>%
  inner_join(Figs3b_DEG_dir_wide %>% select(gene_symbol, direction = `Clec7a+`), 
...
Figs3b_DEG_dir_wide_ordered <- Figs3b_DEG_dir_wide %>%
...
gene_list_phago <- c("Havcr2", "Cd9", "Cst7", "Cstb", "Cxcl16", "Ly9", "Lyz2", 
                     unique(subset(Figs3b_DEG_dir_wide, str_detect(gene_symbol, "H2\\-"))$gene_symbol))

label_df <- Figs3b_DEG_dir_wide_ordered %>%
...
row_annot <- rowAnnotation(
  `Tim3KO` = Figs3b_DEG_dir_wide_ordered$`Tim3KO`,
  `phago+` = Figs3b_DEG_dir_wide_ordered$`phago+`,
  `Tgfbr2KO` = Figs3b_DEG_dir_wide_ordered$`Tgfbr2KO`,
  `Clec7a+` = Figs3b_DEG_dir_wide_ordered$`Clec7a+`,
...
png(paste0("figures/Fig.s3b_overlapping_DEGs_expr_heatmap.png"), width = 10, height = 8.5, units = "in", res = 400)
draw(p, padding = unit(c(2, 2, 10, 2), units = "mm"))
dev.off()
pdf(paste0("figures/Fig.s3b_overlapping_DEGs_expr_heatmap.pdf"), width = 10, height = 8.5)
draw(p)
dev.off()
